...
class SearchAPIClient:
    ...
    def _fetch_page(
        ...
        for attempt in range(3):
            try:
                resp = session.fetch(url, network_idle=network_idle, wait=wait)
    ...
    def search_all_pages(
        ...
        首页完整加载（过 WAF），后续页轻量导航（跳过 network_idle）。
        """
        all_items: list[dict] = []

        # 首页: 完整加载
        page1 = self._fetch_page(session, keyword, job_area, 1,
                                 network_idle=True, wait=1000)
        if page1 is None:
            return all_items

        total_count = get_total_count(page1)
        ...
        first_items = parse_api_response(page1)
        all_items.extend(first_items)
        if on_page:
            on_page(1, pages, first_items)

        if pages <= 1:
            return all_items

        # 后续页: 轻量导航，跳过 network_idle
        for p in range(2, pages + 1):
            data = self._fetch_page(session, keyword, job_area, p,
                                    network_idle=False, wait=300)
            if data is None:
                logger.warning("[%s][%s] 第%d页失败", keyword, job_area, p)
                continue

            items = parse_api_response(data)
            all_items.extend(items)
            if on_page:
                on_page(p, pages, items)

            if p % 10 == 0:
                logger.info(
                    "[%s][%s] 翻页: %d/%d, 累计 %d 条",
                    keyword, job_area, p, pages, len(all_items),
                )

        logger.info(
            "[%s][%s] 完成: %d页, %d条",
            keyword, job_area, pages, len(all_items),
        )
        return all_items
